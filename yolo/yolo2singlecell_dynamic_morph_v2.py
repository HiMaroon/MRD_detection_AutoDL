...
本版本在 yolo2singlecell_dynamic_morph.py 基础上补充：
1) 参考 yolo2singlecell.py，基于标注 json 中的 polygon/polyline 与 YOLO 轮廓做 IoU 匹配，
   将匹配得到的 label 写入输出图片文件名末尾。
2) 保留 build_morph_yolo.py 对齐后的 morph 表头。
3) 对部分几何量同时输出“裁剪前(原图坐标)”与“裁剪后(最终保存图像坐标)”两套列。
"""
...
# 用户点名需要同时保存裁剪前/裁剪后的几何量
DUAL_SCALE_GEOM_FEATURES = [
    "area",
    "perimeter",
    "convex_area",
    "equiv_diameter",
...
PRE_CROP_FEATURES = [f"pre_crop_{k}" for k in DUAL_SCALE_GEOM_FEATURES]
POST_CROP_FEATURES = [f"post_crop_{k}" for k in DUAL_SCALE_GEOM_FEATURES]
...
def calculate_contour_iou(contour1: Optional[np.ndarray], contour2: Optional[np.ndarray]) -> float:
    """参考 yolo2singlecell.py：使用掩码法计算任意形状轮廓的 IoU。"""
    if contour1 is None or contour2 is None:
        return 0.0

    x1, y1, w1, h1 = cv2.boundingRect(contour1.astype(np.int32))
    x2, y2, w2, h2 = cv2.boundingRect(contour2.astype(np.int32))
    ...
    contour1_shifted = contour1.astype(np.int32) - np.array([[[min_x, min_y]]], dtype=np.int32)
    contour2_shifted = contour2.astype(np.int32) - np.array([[[min_x, min_y]]], dtype=np.int32)
    ...
    inter_area = int(np.sum(intersection))
...
def load_ground_truth_polygons(points_json_path: Path) -> List[Dict]:
    """参考 yolo2singlecell.py：加载标注 json 中 polygon/polyline 的 contour 与 label。"""
    ...
    try:
        ...
        for shape in data.get("shapes", []):
            if shape.get("shape_type") in ["polygon", "polyline"] and shape.get("points"):
                ...
                gt_polygons.append({"contour": contour, "label": label})
    except Exception as e:
        print(f"加载标注文件失败 {points_json_path}: {e}")
    return gt_polygons


def assign_label_by_iou(
    contour_global: np.ndarray,
    gt_polygons: List[Dict],
    iou_threshold: float,
) -> str:
    """参考 yolo2singlecell.py：取 IoU 最大的 gt label，低于阈值则回退为 0。"""
    cell_label = "0"
    max_iou = 0.0
    for gt in gt_polygons:
        iou = calculate_contour_iou(contour_global, gt.get("contour"))
        if iou > max_iou:
            max_iou = iou
            cell_label = str(gt.get("label", "0"))
...
def process_cells_dynamic(
    ...
    for cell_data in seg_data:
        ...
        # 在原图坐标系下做基础过滤，并保留“裁剪前”的几何量
        pre_crop_contour_feat = compute_contour_features(contour_global)
        if pre_crop_contour_feat["area"] < min_area:
            continue
        if pre_crop_contour_feat["circularity"] < min_circularity:
            continue

        cell_label = assign_label_by_iou(contour_global, gt_polygons, iou_threshold=iou_threshold)
        ...
        # 现有 build_morph_yolo 对齐特征仍按“最终保存图像坐标系”计算
        post_crop_contour_feat = compute_contour_features(contour_for_saved)
        appear_feat = compute_weak_appearance_features(crop_img, contour_for_saved)
        morph = {**post_crop_contour_feat, **appear_feat}

        image_stem = image_path.stem
        out_name = f"{image_stem}_{saved_idx:03d}_{cell_label}.png"
        ...
        # 保持原有 build_morph_yolo 对齐列不变（即当前/裁剪后数值）
        row.update({k: float(morph.get(k, 0.0)) for k in ALL_FEATURES})
        # 额外显式保存裁剪前/裁剪后的指定几何量
        row.update({f"pre_crop_{k}": float(pre_crop_contour_feat.get(k, 0.0)) for k in DUAL_SCALE_GEOM_FEATURES})
        row.update({f"post_crop_{k}": float(post_crop_contour_feat.get(k, 0.0)) for k in DUAL_SCALE_GEOM_FEATURES})
        rows.append(row)
        saved_idx += 1
    ...
    **kwargs,
    ...
    for seg_json_file in tqdm(seg_json_files, desc="动态裁剪中"):
        ...
        rows = process_cells_dynamic(
            ...
            points_json_path=Path(points_json_file),
    ...
    if missing_points > 0:
        print(f"⚠️ 缺少匹配标注 json：{missing_points} 个")
    ...
    fieldnames = fixed_fields + ALL_FEATURES + PRE_CROP_FEATURES + POST_CROP_FEATURES
...
def main():
    ...
    datasets = [
        ...
        #     "out": r"/root/autodl-tmp/projects/myq/SingleCellProject/dataset/singlecell_dynamic_size1.1_260323/test_TJMU",
        # },
    ]
