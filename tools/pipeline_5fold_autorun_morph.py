...
def patch_train_configs(
    ...
    *,
    train_labels: Path,
    val_labels: Path,
    train_morph_csv: Path | None,
    val_morph_csv: Path | None,
    output_root: Path,
    ...
    if train_morph_csv is not None:
        data_cfg["train_morph_csv"] = str(train_morph_csv)
    if val_morph_csv is not None:
        data_cfg["val_morph_csv"] = str(val_morph_csv)
...
def run_two_stage_classifier_train(
    ...
    try:
        ...
        patch_train_configs(
            ...
            train_morph_csv=train_morph_gt,
            val_morph_csv=val_morph_yolo,
        ...
        patch_train_configs(
            ...
            val_labels=val_labels_yolo,
            train_morph_csv=train_morph_yolo,
            val_morph_csv=val_morph_yolo,
...
def run_eval_and_patient_report(
    ...
    morph_map: Dict[str, Path] | None = None,
):
    for split_name, label_path in labels_map.items():
        ...
        run_test_on_split(
            ...
            test_morph_csv=str(morph_map[split_name]) if morph_map and split_name in morph_map else None,
        )
...
def build_morph_csv_for_fold(args, fold_root: Path, data_roots: Dict[str, Path]) -> Dict[str, Path]:
    singlecell_root = fold_root / "singlecell"
    pred_root = fold_root / "yolo_preds"
    morph_root = fold_root / "morph_csv"
    morph_root.mkdir(parents=True, exist_ok=True)

    split_names = ["train_groundtruth", "train", "val", "test_BJH", "test_FXH_noALL", "test_TJMU"]
    splits = []
    for split_name in split_names:
        label_name = f"{split_name}_labels.txt"
        splits.append(
            {
                "name": split_name,
                "label_txt": str(singlecell_root / label_name),
                "seg_root": str(pred_root / ("train" if split_name == "train_groundtruth" else split_name)),
                "orig_root": str(data_roots["train"] if split_name == "train_groundtruth" else data_roots[split_name]),
            }
        )

    with tempfile.NamedTemporaryFile("w", suffix=".json", encoding="utf-8", delete=False) as tf:
        json.dump(splits, tf, ensure_ascii=False, indent=2)
        splits_json = Path(tf.name)
    try:
        run_cmd(
            [
                sys.executable,
                "tools/build_morph_yolo_with_featurewise_norm.py",
                "--splits_json",
                str(splits_json),
                "--out_root",
                str(morph_root),
                ...
                str(args.crop_size),
                "--min_circularity",
                str(args.min_circularity),
                "--min_area",
                str(args.min_area),
                "--max_workers",
                str(args.morph_workers),
                "--qa_max",
                str(args.morph_qa_max),
                "--filter_edge_cells",
            ],
    ...
    return {name: morph_root / f"{name}_morph.csv" for name in split_names}
...
def run_fold_pipeline(args, fold: FoldInfo):
    ...
    if args.reuse_fold_root:
        reuse_root = Path(args.reuse_fold_root) / f"fold{fold.fold_id}"
        print(f"♻️ Fold{fold.fold_id} 复用已有 yolo_preds/singlecell: {reuse_root}")
        if not (reuse_root / "yolo_preds").exists() or not (reuse_root / "singlecell").exists():
            raise FileNotFoundError(f"复用目录缺失 yolo_preds 或 singlecell: {reuse_root}")
        ensure_clean_dir(pred_root)
        ensure_clean_dir(singlecell_root)
        shutil.copytree(reuse_root / "yolo_preds", pred_root, dirs_exist_ok=True)
        shutil.copytree(reuse_root / "singlecell", singlecell_root, dirs_exist_ok=True)
    else:
        run_yolo_predict(
    ...
    morph_map = build_morph_csv_for_fold(args, fold_root, data_roots)
    ...
    final_ckpt = run_two_stage_classifier_train(
        ...
        train_morph_gt=morph_map["train_groundtruth"],
        train_morph_yolo=morph_map["train"],
        val_morph_yolo=morph_map["val"],
    ...
    run_eval_and_patient_report(
        ...
        morph_map={
            "train": morph_map["train"],
            "val": morph_map["val"],
            "test_BJH": morph_map["test_BJH"],
            "test_FXH_noALL": morph_map["test_FXH_noALL"],
            "test_TJMU": morph_map["test_TJMU"],
        },
    )
...
def parse_args():
    ...
    p.add_argument("--morph-workers", type=int, default=16, help="morph 特征构建并行进程数")
    p.add_argument("--morph-qa-max", type=int, default=20, help="morph QA 导出最大样本数")
    p.add_argument("--morph-features", default="contour+weak", help="morph 特征子集，传递给 build_morph_yolo_with_featurewise_norm.py")
    p.add_argument(
        "--cls-init-ckpt",
