...
from tools.test import run_test_on_split, calculate_and_save_metrics
...
def merge_5fold_external_results(output_root: Path, patient_xlsx: Path, external_splits: List[str]):
    """
    将 5-fold 的外部测试结果做模型融合（概率平均）并重新产出评估与患者级报告。
    期望输入文件：{output_root}/fold{1..N}/eval/{split}/val_results.csv
    """
    output_root = Path(output_root)
    set_chinese_font()

    for split_name in external_splits:
        fold_csvs = sorted(output_root.glob(f"fold*/eval/{split_name}/val_results.csv"))
        if len(fold_csvs) == 0:
            print(f"⚠️ 未找到可融合结果，跳过: {split_name}")
            continue

        dfs = []
        for fp in fold_csvs:
            df = pd.read_csv(fp)
            df["fold_from"] = fp.parts[-4]  # fold1/fold2...
            dfs.append(df)

        # 以 image 为键，融合各 fold 的概率
        merged = dfs[0][["image", "true_label"]].copy()
        ...
        if merged.empty:
            print(f"⚠️ 融合后为空（不同 fold 的 image 无交集），跳过: {split_name}")
            continue

        # 平均概率
        for c in prob_cols:
            fold_prob_cols = [f"{c}_f{i}" for i in range(1, len(dfs) + 1)]
            merged[c] = merged[fold_prob_cols].mean(axis=1)
        ...
        keep_cols = ["image", "true_label", "pred_label", "correct"] + prob_cols
        merged = merged[keep_cols].sort_values("image").reset_index(drop=True)

        out_dir = output_root / "ensemble_eval" / split_name
        out_dir.mkdir(parents=True, exist_ok=True)
        csv_path = out_dir / "val_results.csv"
        merged.to_csv(csv_path, index=False)

        # 复用 tools.test 里的指标保存逻辑
        ...
        # 复用患者级统计
        run_one_task(
            cell_result_csv=str(csv_path),
            patient_info_xlsx=str(patient_xlsx),
            output_png=str(output_root / "ensemble_patient_report" / split_name / "patient_ratio_from_cell_results.png"),
            output_excel=str(output_root / "ensemble_patient_report" / split_name / "patient_ratio_from_cell_results.xlsx"),
        )
...
def parse_args():
    ...
    p.add_argument("--yolo-epochs", type=int, default=5)
    p.add_argument("--yolo-batch", type=int, default=32)
    p.add_argument("--yolo-patience", type=int, default=25)
    ...
    p.add_argument(
        "--cls-init-ckpt",
        default="/root/autodl-tmp/projects/myq/SingleCellProject/outputs/260323_gt_576_0.65_2class_onlineAug/epoch=40-val_acc_macro=0.0000.ckpt",
        help="分类训练 Stage1 的默认初始化 ckpt（为空则沿用 model.yaml）",
    )
    p.add_argument(
        "--no-merge-external-after-folds",
        dest="merge_external_after_folds",
        action="store_false",
        help="关闭 5-fold 完成后对外部测试集结果的融合",
    )
    p.set_defaults(merge_external_after_folds=True)
    return p.parse_args()
...
def main():
    ...
    if args.merge_external_after_folds:
        merge_5fold_external_results(
            output_root=out_root,
            patient_xlsx=Path(args.patient_xlsx),
            external_splits=["test_BJH", "test_FXH_noALL", "test_TJMU"],
        )
