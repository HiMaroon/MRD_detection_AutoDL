...
def run_yolo_predict(
    ...
    for split_name, input_root in data_roots.items():
        ...
        for result in tqdm(results, total=len(images), desc=f"yolo_predict:{split_name}"):
...
def parse_args():
    ...
    p.add_argument("--yolo-predict-batch", type=int, default=16, help="YOLO 推理 batch size")
