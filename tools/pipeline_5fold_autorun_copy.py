...
from tools.patient_analysis import run_one_task
...
def run_yolo_predict(model_path: Path, data_roots: Dict[str, Path], pred_root: Path, conf: float = 0.25, iou: float = 0.5, imgsz: int = 640):
    ...
    for split_name, input_root in data_roots.items():
        ...
        for img_path in tqdm(images, desc=f"yolo_predict:{split_name}"):
            result = model(str(img_path), verbose=False, conf=conf, iou=iou, imgsz=imgsz)[0]
...
def patch_train_configs(repo_root: Path, *, train_labels: Path, val_labels: Path, output_root: Path, local_weight_path: Path | None):
    ...
    model_cfg["local_weight_path"] = str(local_weight_path) if local_weight_path else model_cfg.get("local_weight_path")
...
def run_two_stage_classifier_train(repo_root: Path, fold_root: Path, train_labels_gt: Path, train_labels_yolo: Path, val_labels_yolo: Path) -> Path:
    data_yaml = repo_root / "configs" / "data.yaml"
    train_yaml = repo_root / "configs" / "train.yaml"
    model_yaml = repo_root / "configs" / "model.yaml"
...
def run_fold_pipeline(args, fold: FoldInfo):
    ...
    run_yolo_predict(yolo_best, data_roots, pred_root)
...
def parse_args():
    ...
    p.add_argument("--yolo-epochs", type=int, default=10)
